...
contract SwapTwoStepsReserve is ApprovedDigitalCurrency {
   ...
   function startSwap(RealTokenizado tokenSender, RealTokenizado tokenReceiver, address receiver, uint256 amount) public {
        ...
        require(CBDC.authorizedAccounts(tokenReceiver.reserve()), "SwapRealTokenizado: Unknown account");
        require(CBDC.authorizedAccounts(tokenSender.reserve()), "SwapRealTokenizado: Unknown account");
        ...
        require(tokenSender.allowance(_msgSender(),address(this)) >= amount, string.concat("SwapRealTokenizado: ",tokenSender.name()," does not have enough allowance"));
        require(CBDC.allowance(tokenSender.reserve(),address(this)) >= amount, "SwapRealTokenizado: Real Digital not allowed to transfer funds");
        require(tokenSender.authorizedAccounts(address(this)), string.concat("SwapRealTokenizado: Swap not authorized on ",tokenSender.name()));
        require(CBDC.authorizedAccounts(address(this)), "SwapRealTokenizado: Swap not authorized on CBDC");
        ...
        tokenSender.transferFrom(_msgSender(),address(this),amount);
        CBDC.transferFrom(tokenSender.reserve(),address(this),amount);
   ...
   function executeSwap(uint256 proposalId) public {
        require(CBDC.authorizedAccounts(swapProposals[proposalId].tokenReceiver.reserve()), "SwapRealTokenizado: Unknown account");
        ...
        swapProposals[proposalId].tokenSender.burn(swapProposals[proposalId].amount);

        CBDC.transfer(swapProposals[proposalId].tokenReceiver.reserve(), swapProposals[proposalId].amount);
    ...
    function cancelSwap(uint256 proposalId, string calldata reason) public {
        ...
        CBDC.transfer(swapProposals[proposalId].tokenSender.reserve(), swapProposals[proposalId].amount);
        swapProposals[proposalId].tokenSender.transfer(swapProposals[proposalId].sender, swapProposals[proposalId].amount);
