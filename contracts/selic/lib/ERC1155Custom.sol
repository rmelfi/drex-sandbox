...
contract ERC1155Custom is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function safeTransferFrom(address from, address to, uint256 id, uint256 amount, bytes memory data) public virtual override {
        address originalSender = decodeSenderData(data);
        require(from == originalSender || isApprovedForAll(from, originalSender), "ERC1155: caller is not token owner or approved");
    ...
    function _safeTransferFrom(address from, address to, uint256 id, uint256 amount, bytes memory data) internal virtual {
        ...
        address operator = decodeSenderData(data);
     ...
     */
    function _burn(address from, uint256 id, uint256 amount, bytes calldata data) internal virtual {
     ...
     * - `ids` and `amounts` must have the same length.
     */
    function _burnBatch(address from, uint256[] memory ids, uint256[] memory amounts, bytes calldata data) internal virtual {
        require(from != address(0), "ERC1155: burn from the zero address");
        require(ids.length == amounts.length, "ERC1155: ids and amounts length mismatch");
    ...
    function decodeSenderData(bytes memory data) internal view returns (address) {
        if (data.length >= 20) {
            address decodedAddress;
            decodedAddress = abi.decode(data, (address));
