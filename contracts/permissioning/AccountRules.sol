...
contract AccountRules is AccountRulesProxy, AccountRulesList {
    ...
    event AddTarget(string message, address indexed target);
    event RemoveTarget(string message, address indexed target);
    event AddDeployer(string message, address indexed deployer);
    event RemoveDeployer(string message, address indexed deployer);

    mapping(address => bool) public targetsEnabled;
    ...
    modifier onlyAuthorized() {
        require(isAuthorizedAdmin(msg.sender) || (deployers[msg.sender]), "Sender not authorized");
        _;
    }
    ...
    function transactionAllowed(
        ...
        if (isAuthorizedAdmin(sender) || (deployers[sender])) {
            return true;
        }

        return (targetsEnabled[target]);
    }
    ...
    function isAuthorizedDeployer(address user) private view returns (bool) {
        return deployers[user];
    }

    function addTarget(address target) external onlyAuthorized returns (bool){
        targetsEnabled[target] = true;
        emit AddTarget("Contrato permitido: ", target);
        return targetsEnabled[target];
    }

    function removeTarget(address target) external onlyAuthorized {
        delete targetsEnabled[target];
        emit RemoveTarget("Permissao de contrato removida: ", target);
    }
    
    function addDeployer(address deployer) external onlyAdmin returns (bool){
        deployers[deployer] = true;
        emit AddDeployer("Deployer permitido: ", deployer);
        return deployers[deployer];
    }

    function removeDeployer(address deployer) external onlyAdmin {
        delete deployers[deployer];
        emit RemoveDeployer("Permissao de deployer removida: ", deployer);
    }

    function verifyDeployer(address deployer) public view returns (bool) {
        return deployers[deployer];
    }
